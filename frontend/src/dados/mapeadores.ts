import type { BancoConectadoDTO, CategoriaDeGastoDTO, LancamentoDTO } from '@contacomigo/contrato';
...
import { BANCOS, CATEGORIAS } from './apresentacao';
...
function partesEmSaoPaulo(instante: Date) {
  ...
  return { ano: Number(v('year')), mes: Number(v('month')), dia: Number(v('day')), hora: v('hour').padStart(2, '0'), minuto: v('minute') };
}
...
export function rotuloDeData(iso: string, agora: Date): string {
  ...
  if (d.ano === hoje.ano && d.mes === hoje.mes && d.dia === hoje.dia) return `Hoje, ${hora}`;
  if (d.ano === ontem.ano && d.mes === ontem.mes && d.dia === ontem.dia) return `Ontem, ${hora}`;
  return `${d.dia} ${MESES[d.mes - 1]}, ${hora}`;
}

/** "Agora mesmo" | "3 min atrás" | "Sincronizando..." */
export function rotuloDeSincronizacao(iso: string | null, status: BancoConectadoDTO['status'], agora: Date): string {
...
export function paraTransactions(dados: LancamentoDTO[], agora: Date): Transaction[] {
  return dados.map((l) => {
    const categoria = l.categoria ? CATEGORIAS[l.categoria.id] : CATEGORIAS.outros;
    const banco = BANCOS[l.instituicao.id];
    return {
      id: l.id,
      description: l.descricao,
      merchant: l.estabelecimento,
      category: l.categoria?.nome ?? 'Outros',
      categoryIcon: categoria.iconeNaLista,
      bank: l.instituicao.nome,
      bankColor: banco?.cor ?? '#94a3b8',
      amount: l.valorEmCentavos / 100,
      type: l.tipo === 'debito' ? 'debit' : 'credit',
      formattedAmount: valorComSinal(l.valorEmCentavos),
      date: new Date(l.dataDeCompetencia),
      formattedDate: rotuloDeData(l.dataDeCompetencia, agora),
    };
  });
}

export function paraConnectedBanks(dados: BancoConectadoDTO[], agora: Date): ConnectedBank[] {
  return dados.map((b) => {
    ...
    return {
      ...
      initials: visual?.iniciais ?? b.nome.slice(0, 2).toUpperCase(),
      balance: b.saldoEmCentavos / 100,
      formattedBalance: `R$ ${numeroEmReais(b.saldoEmCentavos)}`,
...
export function paraSpendingCategories(dados: CategoriaDeGastoDTO[]): SpendingCategory[] {
  // Percentual derivado dos totais, com uma casa — o mesmo numero que o mock
  // trazia pronto, agora calculado em vez de digitado.
  const total = dados.reduce((s, c) => s + c.totalEmCentavos, 0);
  return dados.map((c) => {
    const visual = CATEGORIAS[c.categoria.id];
    return {
      id: visual?.idLegado ?? c.categoria.id,
      name: c.categoria.nome,
      value: c.totalEmCentavos / 100,
      percentage: total === 0 ? 0 : Math.round((c.totalEmCentavos / total) * 1000) / 10,
      color: visual?.cor ?? '#94a3b8',
