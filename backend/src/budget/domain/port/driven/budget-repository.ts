...
import type { BudgetAlert } from '../../model/budget-alert';
...
export interface BudgetRepository {
  ...
  /** Alerts already emitted for (holder, month) — RN-005: one per band. */
  listarAlertasDoMes(holderId: string, month: string): Promise<readonly BudgetAlert[]>;

  /** Persists an emitted alert so it never repeats for the same band (RN-005). */
  registrarAlerta(alerta: BudgetAlert): Promise<void>;
}
