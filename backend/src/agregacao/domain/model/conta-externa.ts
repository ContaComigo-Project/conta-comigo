// O que o agregador devolve, traduzido para o vocabulario do dominio. Nenhum
// campo do provedor vaza para ca: se o Pluggy renomear um atributo, muda o
// adaptador, nao isto (RNF-020).

export type TipoDeContaExterna = 'corrente' | 'poupanca' | 'cartao-de-credito';

export interface ContaExterna {
  readonly idExterno: string;
  readonly instituicao: string;
  readonly tipo: TipoDeContaExterna;
  /** Em centavos, como todo valor do sistema (RN-006). */
  readonly saldoEmCentavos: number;
}

export interface LancamentoExterno {
  readonly idExterno: string;
  readonly idDaContaExterna: string;
  /** Descricao crua do provedor. A versao legivel e RF-010 (HN-004). */
  readonly descricaoOriginal: string;
  readonly valorEmCentavos: number;
  readonly dataDeCompetencia: Date;
}
