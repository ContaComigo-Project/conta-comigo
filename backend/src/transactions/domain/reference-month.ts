// RN-003 — o mes de referencia de um transaction e o mes civil da sua data de
...
export function mesDeReferencia(data: Date): ReferenceMonth {
  const partes = formatador.formatToParts(data);
  const valor = (tipo: string) => Number(partes.find((p) => p.type === tipo)?.value);
  return { ano: valor('year'), mes: valor('month') };
}

export function mesmoMes(a: ReferenceMonth, b: ReferenceMonth): boolean {
